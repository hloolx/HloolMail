...
func apiDocsMarkdown(cfg apiDocsConfig) string {
	...
	lines := []string{
		...
		"",
		"List active, MX-verified public domain names:",
		"",
		"```bash",
		...
		"| --- | --- | --- | --- |",
		"| `GET` | `/api/domains/available` | API key | List active, MX-verified public domain names in `domains` |",
...
func apiSkillMarkdown(cfg apiDocsConfig) string {
	...
	lines := []string{
		...
		"",
		"1. Call `GET /api/domains/available` with `X-API-Key` to list usable public domains from `data.domains`.",
		...
		"## Core Endpoints",
		"",
		"- `GET /api/domains/available` lists usable public domain names in `data.domains` and requires an API key.",
