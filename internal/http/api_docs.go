...
func apiDocsMarkdown(cfg apiDocsConfig) string {
	...
	lines := []string{
		...
		"| --- | --- | --- | --- |",
		"| `GET` | `/api/domains/available` | API key | List available public domains and API-key-accessible private domains. Legacy public names remain in `domains` |",
		...
		"| `GET` | `/api/email/:id` | API key | Read one message with text body, headers, and `seen` state |",
		...
		"  \"success\": false,",
		"  \"data\": null,",
		"  \"error\": \"domain MX not verified\"",
		"}",
		"```",
		"",
		"Common domain errors include `domain not found`, `domain MX not verified`, `private domain access denied`, and `no available public domains; pass domain to use a private domain`.",
		"",
...
func apiSkillMarkdown(cfg apiDocsConfig) string {
	...
	lines := []string{
		...
		"## Core Endpoints",
		"",
		"- `GET /api/domains/available` lists usable public domains in `data.domains` and `data.public_domains`, plus accessible private domains in `data.private_domains`.",
