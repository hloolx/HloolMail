...
export function AdminAuditPage() {
  const text = useText();
  return (
    <AdminPageFrame title={text.page['admin-audit']}>
      <AdminAuditLog />
    </AdminPageFrame>
  );
}
...
export function AdminAuditLog() {
  ...
  const auditLogs = useQuery({
    queryKey: queryKeys.admin.auditLogs(auditFilters, auditSearch, auditPage, auditPerPage),
    ...
    <section className="panel admin-table-panel" id="admin-audit-logs">
      <div className="panel-header admin-panel-header">
        ...
        <div className="table-actions">
          ...
          >
            <RefreshCw size={14} className={auditLogs.isFetching ? 'animate-spin' : ''} aria-hidden="true" />
            {text.common.refresh}
          </button>
      ...
      <DataTable
        ...
        error={auditLogs.isError}
        errorLabel={queryErrorMessage(auditLogs.error, text.admin.auditLogs.empty)}
        retryLabel={text.common.retry}
        onRetry={() => auditLogs.refetch()}
        retryPending={auditLogs.isFetching}
        columns={columns}
        hiddenColumnKeys={hiddenColumnKeys}
        onHiddenColumnKeysChange={setHiddenColumnKeys}
...
function buildAuditLogQuery(filters: AuditLogFilters, search: string, page: number, perPage: number) {
  ...
  if (filters.targetType !== 'all') params.set('target_type', filters.targetType);
