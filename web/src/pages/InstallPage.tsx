      ...
      <div className="mx-auto grid max-w-5xl gap-4 lg:grid-cols-[1fr_24rem]">
        <section className="panel">
          <div className="panel-header">
            ...
            <StatusPill ok>Install</StatusPill>
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <label className="grid gap-1 text-sm">
              ...
              <select className="input" value={form.database_driver} onChange={(event) => set('database_driver', event.target.value)}>
                <option value="sqlite">SQLite</option>
                <option value="postgres">PostgreSQL</option>
              </select>
            </label>
            <Field label={text.install.databaseURL} value={form.database_url} onChange={(value) => set('database_url', value)} />
            <Field
            ...
            />
            <Field label={text.install.httpAddr} value={form.http_addr} onChange={(value) => set('http_addr', value)} hint={text.install.httpAddrHint} />
            <Field label={text.install.smtpAddr} value={form.smtp_addr} onChange={(value) => set('smtp_addr', value)} hint={text.install.smtpAddrHint} />
          </div>
          <label className="toggle-row mt-3">
            <input type="checkbox" checked={form.dev_mode} onChange={(event) => set('dev_mode', event.target.checked)} />
