...
const ADMIN_WEBHOOK_PAGE_SIZE_OPTIONS = [10, 20, 50, 100];
...
export function AdminWebhooksPanel() {
  ...
  const disable = useDisableAdminWebhookMutation({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: webhookKeys.adminAll });
    ...
    <section className="panel admin-table-panel" id="admin-webhooks">
      ...
      {webhooks.isError ? (
        ...
        <DataTable
          ...
          emptyLabel={webhooks.isLoading ? text.common.loading : text.admin.webhooks.empty}
          columns={columns}
          hiddenColumnKeys={hiddenColumnKeys}
          onHiddenColumnKeysChange={setHiddenColumnKeys}
            ...
            cells: [
              ...
              <div className="table-actions" data-webhook-id={endpoint.id}>
                ...
                <IconButton
                  ...
                  onClick={() => {
                    if (window.confirm(text.admin.webhooks.disableConfirm.replace('{target}', webhookTargetText(endpoint, text)))) {
                      disable.mutate(endpoint);
                    }
                  }}
                  disabled={disable.isPending || !endpoint.enabled}
                >
                  <ShieldOff size={14} />
                </IconButton>
                <IconButton
                  ...
                  onClick={() => {
                    if (window.confirm(text.admin.webhooks.deleteConfirm.replace('{target}', webhookTargetText(endpoint, text)))) {
                      deleteWebhook.mutate(endpoint);
                    }
                  }}
