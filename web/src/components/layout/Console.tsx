...
const AdminOAuthPage = lazy(() => import('../../pages/AdminOAuthPage').then(m => ({ default: m.AdminOAuthPage })));
import { Sidebar } from './Sidebar';
import { Topbar } from './Topbar';

export function Console({ user }: { user: User }) {
  const { page, setPage, sidebarCollapsed } = useAppStore();
  const visiblePage: Page = user.role !== 'admin' && (page === 'admin' || page === 'users' || page === 'admin-oauth') ? 'inbox' : page;
          ...
          <div className="main-content px-4 py-4 sm:px-6">
            <AnimatePresence mode="wait">
              <Suspense fallback={<div className="page-transition-wrapper" />}>
                ...
                {visiblePage === 'admin-oauth' && <AdminOAuthPage />}
